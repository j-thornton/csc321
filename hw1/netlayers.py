...
linux_mac_conv = macos_mac_conv

# ----------------------------------------------------------------------
# ARP
# ----------------------------------------------------------------------

# ? (192.168.1.207) at 28:f0:76:54:e3:1e on en1 ifscope permanent [ethernet]
...
# ? (172.17.0.1) at 02:42:b0:04:27:f0 [ether] on eth0
arp_output_linux = _.partial(getoutput, 'arp -a')
arp_linux_re = re.compile(
    fr'^(?P<name>[?.\w-]*)\s+\((?P<ip>{ip_re})\) at (?P<mac>[A-Fa-f0-9:]*?)(?:\s+.*?) on .*$'
)
...
get_arp_linux = get_arp(arp_output_linux, arp_linux_re, linux_mac_conv)
...
ping_output, ping_re = {
    ...
    'Linux': (ping_output_macos, ping_re_macos),
}[platform.system()]
